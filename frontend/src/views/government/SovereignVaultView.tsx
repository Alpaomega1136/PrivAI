    ...
    <div className="view-stack sovereign-vault-view">
      <section className="vault-hero-card">
        ...
        <div className="vault-hero-copy">
          ...
          <p>
            Original disimpan sebagai encrypted bundle. Halaman ini hanya menunjukkan metadata, status wrapping key, dan
            simulasi rotasi key tanpa pernah mengekspos plaintext atau private key.
          </p>
        </div>
        ...
        <div className="vault-rotation-stage">
          ...
          {rotateResult && (
            <div className={`vault-rotation-result ${rotateResult.ok ? "success" : "error"}`}>
              {rotateResult.ok
                ? "Key rotation berhasil. Upload baru memakai active key terbaru, sementara key lama tetap tersimpan untuk bundle lama."
      ...
      <section className="vault-record-workbench">
        <aside className="vault-record-browser">
          ...
          <label className="vault-search-field" aria-label="Cari vault record">
            <Search size={18} />
            <input
              type="text"
              placeholder="Cari record ID, filename, session, atau key..."
              value={search}
          ...
          {records.length === 0 ? (
            <div className="vault-empty-detail">Belum ada original terenkripsi di vault.</div>
          ) : filteredRecords.length === 0 ? (
            <div className="vault-empty-detail">Tidak ada record vault yang sesuai pencarian.</div>
          ) : (
        ...
        <section className="vault-metadata-panel">
          ...
          {vaultResult && !vaultResult.ok && (
            <div className="result-box error-box">{vaultResult.error?.message || "Metadata vault tidak ditemukan."}</div>
          )}
          ...
          {metadataSource ? (
            ...
            <div className="vault-empty-detail large">
              <LockKeyhole size={30} />
              <strong>Pilih record vault</strong>
              <span>Metadata terenkripsi akan tampil di sini tanpa membuka plaintext original.</span>
            </div>
          )}
        </section>
      </section>
